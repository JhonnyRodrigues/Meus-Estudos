...
SP_GERAR_CREDITOS_VENDA: BEGIN
    ...
    DECLARE V_RESTO INT;
    DECLARE V_VAL_PARCELA_CENTAVOS INT;
    DECLARE V_VAL_PARCELA DECIMAL(10,2);
    DECLARE V_ID_PARCELA INT;
    DECLARE V_VENCIMENTO DATE;
    DECLARE V_ULTIMO_CREDITO_INSERIDO INT;
    ...
    SELECT VALOR_TOTAL, QUANTIDADE_PARCELAS, DATA_PRIMEIRO_VENCIMENTO, FK_FORMA_PAGAMENTO_PREVISTA
    INTO V_TOTAL_VENDA, V_PARCELAS, V_DATA_PRIMEIRO_VENCIMENTO, V_FORMA_PAGAMENTO
    FROM VENDAS
    WHERE ID = P_VENDA_ID;
    ...
    IF V_NATUREZA_PAGAMENTO = 'V' THEN
        ...
        VALUES (P_VENDA_ID, 1, V_PAGO, V_TOTAL_VENDA, DATE(NOW()), V_DATA_PRIMEIRO_VENCIMENTO);
        SET V_ULTIMO_CREDITO_INSERIDO = LAST_INSERT_ID();
    
        INSERT INTO RECEBIMENTOS (FK_CREDITO_RECEBER, VALOR_PAGO, DATA_PAGAMENTO, FK_FORMA_PAGAMENTO)
        VALUES (V_ULTIMO_CREDITO_INSERIDO, V_TOTAL_VENDA, DATE(NOW()), V_FORMA_PAGAMENTO);
    ELSE
        -- PARCELADO: DISTRIBUIR CENTAVOS CORRETAMENTE
        SET V_TOTAL_VENDA_CENTAVOS = ROUND(V_TOTAL_VENDA * 100);
        SET V_BASE_CENTAVOS = FLOOR(V_TOTAL_VENDA_CENTAVOS / V_PARCELAS);
        SET V_RESTO = V_TOTAL_VENDA_CENTAVOS - V_BASE_CENTAVOS * V_PARCELAS;
        SET V_ID_PARCELA = 1;
    
        WHILE V_ID_PARCELA <= V_PARCELAS DO
            SET V_VAL_PARCELA_CENTAVOS = V_BASE_CENTAVOS;
        
            IF V_ID_PARCELA <= V_RESTO THEN
                SET V_VAL_PARCELA_CENTAVOS = V_VAL_PARCELA_CENTAVOS + 1;
            END IF;
        
            SET V_VAL_PARCELA = V_VAL_PARCELA_CENTAVOS / 100;
            SET V_VENCIMENTO = DATE_ADD(V_DATA_PRIMEIRO_VENCIMENTO, INTERVAL (V_ID_PARCELA - 1) MONTH);
        
            INSERT INTO CREDITOS_RECEBER (FK_VENDA, ID_PARCELA, FK_SITUACAO, VALOR_ORIGINAL, DATA_EMISSAO, DATA_VENCIMENTO)
            VALUES (P_VENDA_ID, V_ID_PARCELA, V_ABERTA, V_VAL_PARCELA, DATE(NOW()), V_VENCIMENTO);
...
/* Atualiza automaticamente ESTOQUES.QUANTIDADE_ATUAL ao movimentar um produto */
...
BEGIN
    ...
    -- VALIDAÇÃO: SE PRODUTO ESTIVER INATIVO, BLOQUEIA A MOVIMENTAÇÃO
    IF V_ATIVO = 'N' THEN
...
BEGIN
    ...
    IF OLD.FK_SITUACAO = V_RESERVADA AND (NEW.FK_SITUACAO = V_CANCELADA OR NEW.FK_SITUACAO = V_DEVOLVIDA_PARCIALMENTE) THEN
        UPDATE PRE_VENDA_ITENS SET RESERVADO = 'N' WHERE FK_PRE_VENDA = NEW.ID AND RESERVADO = 'S';

        INSERT INTO ESTOQUE_MOVIMENTACOES (FK_PRODUTO, QUANTIDADE, FK_ACAO, FK_ORIGEM, ID_TRANSACAO_ORIGEM)
        SELECT FK_PRODUTO, QUANTIDADE, V_ENTRADA, V_PREVENDA, FK_PRE_VENDA
...
/* Impedir alteração quando situação CANCELADA */
CREATE TRIGGER TRG_VENDAS_BLOQUEAR_ATUALIZACAO_QUANDO_CANCELADA
BEFORE UPDATE ON VENDAS
FOR EACH ROW
BEGIN
    DECLARE V_CANCELADA INT;
    SET V_CANCELADA = 3; -- ID da situação CANCELADA conforme padrão do sistema
...
-- ÍNDICES (melhoram desempenho em JOINs)
...
/* Evita criar duas vezes a mesma parcela para a mesma venda */
