...
Swal.fire({
	...
	.then((result) => {
		if (result.hasOwnProperty('value') && result.value.trim() !== '') {
			...
			// Configurações da requisicao
			var requestOptions = {
				method: 'POST', //verbo HTTP
				headers: {
					'Content-Type': 'application/x-www-form-urlencoded' // Tipo de conteúdo esperado pela request
				},
			...
			fetch('../SISTEMA_TESTAR_ENVIO_EMAILS_B/index.php', requestOptions) //o retorno dessa função é uma Promise
				.then(response => {
					...
					return response.json(); // o método json() retorna uma Promise que, após parsing(conversão) de uma string JSON, vira um objeto.
				})
				// .then(() => {
